...
class Bitverse:
                    ...
                    async with session.get(url, params=params, proxy=proxy, proxy_auth=proxy_auth) as response:
                        ...
                        if data.get("status") == 200:
                            ...
                            self.log(
                                ...
                                f"{Fore.YELLOW+Style.BRIGHT} No route found (attempt {attempt + 1}) {Style.RESET_ALL}"
                            )
            except Exception as e:
                self.log(
                    f"{Fore.CYAN+Style.BRIGHT}   Status  :{Style.RESET_ALL}"
                    f"{Fore.YELLOW+Style.BRIGHT} Route request failed (attempt {attempt + 1}): {str(e)} {Style.RESET_ALL}"
                )
    ...
    async def perform_swap(self, account: str, address: str, use_proxy: bool):
        ...
        try:
            ...
            if balance < self.swap_amount + 0.002:  
                self.log(
                    f"{Fore.CYAN+Style.BRIGHT}   Status  :{Style.RESET_ALL}"
                    f"{Fore.YELLOW+Style.BRIGHT} Insufficient PHRS balance: {balance:.6f} {Style.RESET_ALL}"
                )
                return None, None
            ...
            if not route:
                self.log(
                    f"{Fore.CYAN+Style.BRIGHT}   Status  :{Style.RESET_ALL}"
                    f"{Fore.RED+Style.BRIGHT} Failed to get swap route {Style.RESET_ALL}"
                )
                return None, None
    ...
    def print_swap_question(self):
        while True:
            try:
                swap_amount = float(input(f"{Fore.YELLOW + Style.BRIGHT}Enter PHRS Swap Amount (default 0.001) -> {Style.RESET_ALL}").strip() or "0.001")
                if swap_amount > 0:
    ...
    def print_question(self):
        while True:
            try:
                ...
                print(f"{Fore.WHITE + Style.BRIGHT}5. PHRS → USDT Swap{Style.RESET_ALL}")
    ...
    async def process_perform_swap(self, account: str, address: str, use_proxy: bool):
        """Process PHRS to USDT swap"""
        tx_hash, block_number = await self.perform_swap(account, address, use_proxy)
        if tx_hash and block_number:
            self.log(
                f"{Fore.CYAN+Style.BRIGHT}   Status  :{Style.RESET_ALL}"
                f"{Fore.GREEN+Style.BRIGHT} Success {Style.RESET_ALL}"
    ...
    async def process_option_5(self, account: str, address: str, use_proxy):
        ...
        self.log(
            f"{Fore.CYAN+Style.BRIGHT}   Amount  :{Style.RESET_ALL}"
            f"{Fore.WHITE+Style.BRIGHT} {self.swap_amount} PHRS {Style.RESET_ALL}"
        )
        ...
        if balance is None:
            self.log(
                f"{Fore.CYAN+Style.BRIGHT}   Status  :{Style.RESET_ALL}"
                f"{Fore.RED+Style.BRIGHT} Fetch PHRS Balance Failed {Style.RESET_ALL}"
            )
            return
        ...
        await self.process_perform_swap(account, address, use_proxy)
    ...
    async def process_accounts(self, account: str, address: str, option: int, use_proxy: bool, rotate_proxy: bool):
        ...
        if is_valid:
            ...
            if option == 1:
                ...
                self.log(
                    f"{Fore.CYAN+Style.BRIGHT}Option  :{Style.RESET_ALL}"
                    f"{Fore.BLUE+Style.BRIGHT} PHRS → USDT Swap {Style.RESET_ALL}"
                )
